...
MySQL.prototype.formatAttributeList = function(attributes) {
    var data = {};
    var sql = '';
    if (typeof attributes == 'string') {
            ...
            attributes.forEach(function (attr, index) {
                sql += (index == attributes.length - 1) ? attr : attr + ', ';
            });
    } else {
        data.error = new Error('attributes must be string or array of strings');
    }
    data.sql = sql;
    return data;
}
...
MySQL.prototype.all = function all(model, filter, callback) {
    ...
    if (filter && filter.attributes) {
        attrObject = this.formatAttributeList(filter.attributes);
        sql += attrObject.sql;
        error = attrObject.error;
    } else {
       sql+= '* ';
    }
        ...
        if (filter && filter.attributes) {
            if (typeof filter.attributes == 'string') {
                ...
                data.forEach(function (value, index) {
                    valueArray.push(value[filter.attributes]);
                });
                objs = valueArray;
            } else {
                objs = data;
