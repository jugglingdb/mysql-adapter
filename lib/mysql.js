...
exports.initialize = function initializeSchema(schema, callback) {
    ...
    var reconnectOnError = s.reconnectOnError || true;
    var reconnectOnErrorTimeout = s.reconnectOnErrorTimeout || 2000;
    ...
    function createConnection() {
        ...
        schema.client.on('error', function (err) {
            if (err.code == 'PROTOCOL_CONNECTION_LOST' && reconnectOnError) {
                console.log('reconnecting in', reconnectOnErrorTimeout, 'ms');
                schema.emit('reconnecting', err);
                setTimeout(createConnection.bind(self), reconnectOnErrorTimeout);
                return;
            }
