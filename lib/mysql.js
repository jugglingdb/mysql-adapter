...
MySQL.prototype.propertiesSQL = function (model) {
    var self = this;
    // Unique index on below was pointless in mysql (created an additional index with no purpose), so removed.
    ...
    Object.keys(this._models[model].properties).forEach(function (prop) {
        if (prop === 'id') return;
        var si = self._models[model].properties[prop];
        if(si.index && si.index === true){
            sql.push('INDEX `' + prop.toUpperCase() + '` (' + prop + ') ');
        }
    });
    if(this._models[model].settings.indexes){ // This in theory is mostly multi-column, but doesn't have to be, just not redundant, I think.
        // Recycled from alterTable.
...
MySQL.prototype.indexSettingsSQL = function (model, prop) {
    // Calling Object.keys changes scope. Hence self rather than this.
...
MySQL.prototype.propertySettingsSQL = function (model, prop) {
    ...
    // This is ugly. But following cue of allowNull.
    return datatype(p) + ' ' +
    (p.allowNull === false || p['null'] === false ? 'NOT NULL' : 'NULL') + ' ' +
    (p.unique && p.unique === true ? 'UNIQUE' : '') + ' ' +
    (p.default && p.default.length > 0  ? 'DEFAULT ' + p.default : '');
};
...
function datatype(p) {
    ...
    switch (p.type.name) {
        ...
        case 'Decimal':
        dt = 'DECIMAL(' + ( (p.limit[0] + "," + p.limit[1]) || "9,2")  + ')';
        break;
    }
    return dt;
}
