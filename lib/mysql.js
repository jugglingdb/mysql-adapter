...
exports.initialize = function initializeSchema(schema, callback) {
    ...
    types.forEach(function(type){
        schema.constructor.registerType(type);
    });
};
...
MySQL.prototype.toDatabase = function (prop, val) {
    ...
    if (prop.type.name === 'Date' || prop.type.name === 'Timestamp') {
        if (!val) return 'NULL';
        if (String(val).trim().toUpperCase() == 'CURRENT_TIMESTAMP') return val;
        if (!val.toUTCString) {
            val = new Date(val);
        }
...
MySQL.prototype.fromDatabase = function (model, data) {
    ...
    Object.keys(data).forEach(function (key) {
        ...
        if (props[key]) {
            ...
            } else if (props[key].type.name === 'Timestamp' && val !== null) {
...
function options(p) {
    ...
    switch (p.type.name) {
        ...
        // Since javascript 'Number' can be a float, seems like it should map to FLOAT in mysql.
        case 'Number': 
        case 'Float':
        ...
        case 'Timestamp':
        p.pass = true;
        if(p.hasOwnProperty('onUpdate')){
            opt += ' ON UPDATE ' + p.onUpdate; // for on update CURRENT_TIMESTAMP
        }
        
        case 'Date':
        if(p.hasOwnProperty('default')){
            if(p.default instanceof Date){
                opt += ' DEFAULT ' + this.schema.client.escape(p.default.toISOString());
            } else {
...
function datatype(p) {
    ...
    switch (p.type.name) {
        ...
        case 'MediumUint':
        dt = 'MEDIUMINT(' + (p.limit || 8) + ') UNSIGNED';
        break;
