...
MySQL.prototype.all = function all(model, filter, callback) {
    ...
    if (filter) {
        ...
        if (filter.where) {
<<<<<<< HEAD
            sql += ' ' + buidquery.buildWhrSet('Where',filter.where,this,props);
=======
            sql += ' WHERE ' + buildWhere(filter.where);
>>>>>>> 725e457992567bfe3835137e427deac3f600f8c5
        }
    ...
    function buildWhere(conds) {
        ...
        Object.keys(conds).forEach(function (key) {
            ...
            if (conds[key] === null || conds[key] === undefined) {
				...
				var sqlCond = '(' + buildWhere(val[0]);
                var i;
				for (i = 1; i < val.length; i++) {
					sqlCond += ' ' + key + ' ' + buildWhere(val[i]);
				}
