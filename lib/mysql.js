...
MySQL.prototype.query = function (sql, callback, quiet) {
    ...
    this.client.query(sql, function (err, data) {
        if (err) {
            if (err.fatal) {
                console.log('fatal mysql error while querying "%s". throwing', sql);
...
MySQL.prototype.toDatabase = function (prop, val, key, model) {
    if (key === 'id' && this.get(model, 'uuid') in {v1: 1, v4: 1} && typeof val !== 'object') {
...
MySQL.prototype.count = function count(model, where, callback) {
    var self = this;
