...
MySQL.prototype.select = function all(model, filter, callback) {

    var sql = 'SELECT * '
    if (filter && filter.attributes) {
        sql = sql.replace(' * ', ' ');
        filter.attributes.forEach(function (attr, index) {
            sql += (index == filter.attributes.length - 1) ? attr : attr + ', ';
        });
    }
        ...
        if (filter && filter.attributes) {
            if (filter.attributes.length == 1) {
                ...
                data.forEach(function (value, index) {
                    valueArray.push(value[filter.attributes[0]]);
                });
                objs = valueArray;
            } else {
                objs = data;
