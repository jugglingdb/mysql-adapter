...
MySQL.prototype.propertiesSQL = function (model) {
    ...
    Object.keys(this._models[model].properties).forEach(function (prop) {
        if (prop === 'id') return;
        sql.push('`' + prop + '` ' + self.propertySettingsSQL(model, prop, indexes));
    });
    indexes.forEach(function(index){
        sql.push('KEY `' + index + '`(`' + index + '`)');
    });
    return sql.join(',\n  ');

};

MySQL.prototype.propertySettingsSQL = function (model, prop, indexes) {
    var p = this._models[model].properties[prop];
    if(p.index && p.index === true){
        indexes.push(prop);
    }
