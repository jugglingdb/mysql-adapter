...
MySQL.prototype.count = function count(model, callback, where) {
    var self = this;
    var props = this._models[model].properties;
    var sql = 'SELECT count(*) AS cnt FROM ' + this.tableEscaped(model);

    if (where) {
        sql += ' WHERE ' + buildWhere(self, where, model);
    }
...
MySQL.prototype.all = function all(model, filter, callback) {
    ...
    if (filter) {
        ...
        if (filter.where) {
            sql += ' WHERE ' + buildWhere(self, filter.where, model);
        }
...
function buildWhere(self, conds, model) {
    var cs = [];
    ...
    Object.keys(conds).forEach(function (key) {
        ...
        if (conds[key] === null || conds[key] === undefined) {
            ...
            conds[key].forEach(function (cond) {
                queries.push(buildWhere(self, cond, model));
            });
