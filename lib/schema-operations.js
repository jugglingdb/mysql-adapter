...
module.exports = function(informationSchema, db, settings) {
    ...
    function buildAlterTableSQL(model, actualFields, actualIndexes) {
        ...
        function formatIndexDefinition(indexName, kind, type, columns) {
            type = type ? `USING ${ type }` : '';
            const name = escapeKey(indexName);
            const cols = columns.map(escapeKey).join(', ');

            return `ADD ${ kind || '' } INDEX ${ name } ${ type } (${ cols })`;
        }
